...
class cylinder_asym(nn.Module):
    ...
    def forward(self, train_pt_fea_ten, train_vox_ten, batch_size):
        coords, features_3d = self.cylinder_3d_generator(train_pt_fea_ten, train_vox_ten)  # [bs, [N, 9]], [bs, [N, 3]] -> [uniq, 4], [uniq, 16]
        spatial_features = self.cylinder_3d_spconv_seg(features_3d, coords, batch_size)  # [uniq, 16], [uniq, 4], bs -> [bs, cls, 480, 360, 32]
