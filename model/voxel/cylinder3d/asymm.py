...
class ResBlock(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, kernel_size=(3, 3, 3), stride=1, pooling=True, drop_out=True, height_pooling=False, indice_key=None):
...
class Asymm_3d_spconv(nn.Module):
    ...
    def __init__(self, output_shape, use_norm=True, num_input_features=128, nclasses=20, n_height=32, strict=False, init_size=16):
        super(Asymm_3d_spconv, self).__init__()
        self.nclasses = nclasses  # semkitti: 19+1; nuscenes: 16+1
        self.nheight = n_height  # 32
        self.strict = False

        sparse_shape = np.array(output_shape)  # [480, 360,  32]
        print("Voxel partition shape: '{}'.".format(sparse_shape))
        self.sparse_shape = sparse_shape  # [480, 360,  32]
    ...
    def forward(self, voxel_features, coors, batch_size):  # voxel_features: [uniq, 16], coors: [uniq, 4], batch_size: bs
        ...
        # import pdb
        # pdb.set_trace()
        ret = spconv.SparseConvTensor(voxel_features, coors, self.sparse_shape, batch_size)  # [uniq, 16], [uniq, 4], (3,), bs -> [480, 360, 32]
        ...
        up0e.features = torch.cat((up0e.features, up1e.features), 1)  # [uniq, 64]
